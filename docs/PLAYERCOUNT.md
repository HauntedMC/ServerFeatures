...
Server names may contain underscores because the metric suffix is parsed from the right.

Health and diagnostics:

- `%playercount_available%` — `true` only while a fresh validated snapshot exists.
- `%playercount_stale%` — `true` when a received snapshot has expired.
- `%playercount_age_seconds%` — age since local receipt, or `-1` before the first snapshot.
...
logical self-healing subscription handle and are closed during feature shutdown.
