...
public class CustomStonecutterRecipe extends AbstractCustomRecipe {
    ...
    public RecipeData createRecipe(JavaPlugin plugin, NamespacedKey key, Map<?, ?> config) {
        ...
        if (!config.containsKey("input")) {
            plugin.getLogger().warning("Stonecutter recipe " + key.toString() + " missing input.");
            return null;
        }
        ...
        try {
            ...
            plugin.getLogger().warning("Unknown input material in stonecutter recipe " + key.toString() + ": " + inputStr);
            return null;
        }
