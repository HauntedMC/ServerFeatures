...
public class CustomSmokingRecipe extends AbstractCustomRecipe {
    ...
    public RecipeData createRecipe(CustomRecipes feature, NamespacedKey key, Map<?, ?> config) {
        ...
        if (!config.containsKey("input")) {
            feature.getLogger().warning("Smoking recipe " + key.toString() + " missing input.");
            return null;
        }
        ...
        try {
            ...
            feature.getLogger().warning("Unknown input material in smoking recipe " + key.toString() + ": " + inputStr);
            return null;
        }
