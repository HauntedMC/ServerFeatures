...
public class CustomFurnaceRecipe extends AbstractCustomRecipe {
    ...
    public RecipeData createRecipe(CustomRecipes feature, NamespacedKey key, Map<?, ?> config) {
        ...
        if (!config.containsKey("input")) {
            feature.getLogger().warning("Furnace recipe " + key.toString() + " missing input.");
            return null;
        }
        ...
        try {
            ...
            feature.getLogger().warning("Unknown input material in furnace recipe " + key.toString() + ": " + inputStr);
            return null;
        }
