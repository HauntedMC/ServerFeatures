...
public final class MessagePlaceholders {
    ...
    public static String applyPlaceholders(String message, MessagePlaceholders placeholders) {
        ...
        for (Map.Entry<String, MessagePlaceholders.Value> e : placeholders.entries().entrySet()) {
            if (!e.getValue().isComponent()) {
                out = out.replace(MessagePlaceholders.token(e.getKey()), e.getValue().asEscapedString());
            }
            if (e.getValue().isComponent()) {
                Component comp = e.getValue().asComponentOrNull();
                String mm = MiniMessage.miniMessage().serialize(comp);
                out = out.replace(MessagePlaceholders.token(e.getKey()), mm);
            }
        }
        return out;
    }
    ...
    private static String token(String key) { return "{" + key + "}"; }

    /** Escapes a value for safe MiniMessage injection. */
    private static String escapeMM(String s) { return MiniMessage.miniMessage().escapeTags(s); }

    // ---- Value kinds ----
    private sealed interface Value permits StrVal, NumVal, CompVal {
        default boolean isComponent() { return this instanceof CompVal; }
        default Component asComponentOrNull() { return (this instanceof CompVal cv) ? cv.value() : null; }
    ...
    private static final class StrVal implements Value {
        private final String value; public StrVal(String value) { this.value = value == null ? "" : value; }
        public String value() { return value; }
    }
    private static final class NumVal implements Value {
        private final Number value; public NumVal(Number value) { this.value = value == null ? 0 : value; }
        public Number value() { return value; }
    }
    private static final class CompVal implements Value {
        private final Component value; public CompVal(Component value) { this.value = Objects.requireNonNull(value, "component"); }
        public Component value() { return value; }
    }
    ...
    private Map<String, Value> entries() { return values; }
    private boolean isEmpty() { return values.isEmpty(); }
    ...
    public static final class Builder {
        ...
        public Builder addString(String key, String value)      { map.put(key, new StrVal(value)); return this; }
        public Builder addNumber(String key, Number value)      { map.put(key, new NumVal(value)); return this; }
        public Builder addComponent(String key, Component value){ map.put(key, new CompVal(value)); return this; }
        ...
        public MessagePlaceholders build() { return new MessagePlaceholders(new LinkedHashMap<>(map)); }
    }
}
