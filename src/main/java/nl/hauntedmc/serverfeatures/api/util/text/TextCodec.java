 ...
 * </ul>
 *
 * <p><b>Why:</b> Keep all format conversions in one place. Use {@link ComponentCodec} for string → {@code Component} parsing.</p>
 *
...
public final class TextCodec {
     ...
     */
    public static Converter convert(String input) { return new Converter(input); }

    /** Shortcut: normalize to MiniMessage using {@link Input#MIXED_INPUT} expectations. */
    public static String toMiniMessage(String input) {
        return convert(input).expect(Input.MIXED_INPUT).toMiniMessage();
    }

    /** Shortcut: normalize and serialize to legacy using '&amp;' codes. */
    public static String toLegacyAmpersand(String input) {
        return convert(input).expect(Input.ANY).toLegacy('&');
    }
    ...
    public static String toPlain(String input) {
        return convert(input).expect(Input.ANY).toPlain();
    }
    ...
    public enum Input {
        ...
        HEX_MINI_DOUBLE,
        /** Bare {@code #RRGGBB} token (word-bounded). */
        HEX_PLAIN,
        ...
        public static final Set<Input> ANY_LEGACY = EnumSet.of(
                LEGACY_AMPERSAND, LEGACY_SECTION, HEX_POUND, HEX_BUNGEE_AMP, HEX_BUNGEE_SECTION
        );
        /** All known hex shapes. */
        public static final Set<Input> HEX_ALL = EnumSet.of(
                HEX_POUND, HEX_BUNGEE_AMP, HEX_BUNGEE_SECTION, HEX_MINI, HEX_MINI_DOUBLE, HEX_PLAIN
        );
        /** Safe default for “mixed” strings (legacy + MiniMessage). */
        public static final Set<Input> MIXED_INPUT = EnumSet.of(
                LEGACY_AMPERSAND, LEGACY_SECTION, HEX_POUND, HEX_BUNGEE_AMP, HEX_BUNGEE_SECTION,
                HEX_MINI, MINIMESSAGE
        );
        /** Everything. */
        public static final Set<Input> ANY = EnumSet.allOf(Input.class);
    }
    ...
    public static final class Options {
        ...
        public static Options all() { return builder().build(); }
        ...
        public static final class Builder {
            ...
            public Builder normalizeSectionToAmpersand(boolean v){ this.normalizeSectionToAmpersand = v; return this; }
            /** Convert legacy hex to {@code <#RRGGBB>} (default: true). */
            public Builder transformHex(boolean v)               { this.transformHex = v; return this; }
            /** Convert standard {@code &/§} codes to MiniMessage tags (default: true). */
            public Builder transformStandardCodes(boolean v)     { this.transformStandardCodes = v; return this; }
            /** Emit reset on closing tags when serializing to legacy (default: true). */
            public Builder legacyEmitResetOnClose(boolean v)     { this.legacyEmitResetOnClose = v; return this; }
            /** Keep hex colors in legacy output (default: true). */
            public Builder legacyOutputHexColors(boolean v)      { this.legacyOutputHexColors = v; return this; }
            /** Use Bungee’s {@code &x} hex format in legacy output (default: false). */
            public Builder xRepeatedHex(boolean v)               { this.xRepeatedHexForLegacyOutput = v; return this; }
    ...
    public static final class Converter {
        ...
        private final EnumSet<Input> expects = EnumSet.noneOf(Input.class);
        private Options options = Options.all();
        private UnaryOperator<String> preprocessor; // e.g., PAPI

        private Converter(String input) { this.originalInput = input; }

        /**
         * Declare expected input formats (multiple allowed). Defaults to {@link Input#MIXED_INPUT}.
         * @param formats expected formats to optimize and guide conversion
         * @return this builder
         */
        public Converter expect(Input... formats) {
            if (formats != null) for (Input f : formats) if (f != null) expects.add(f);
            return this;
        }
         ...
         * @return this builder
         */
        public Converter expect(Set<Input> formats) {
        ...
        public String toMiniMessage() {
            ...
            Set<Input> exp = expects.isEmpty() ? Input.MIXED_INPUT : expects;
    ...
    private static final Pattern POUND_HEX           = Pattern.compile("(?i)&#([0-9a-f]{6})");
    private static final Pattern SECTION_POUND_HEX   = Pattern.compile("(?i)§#([0-9a-f]{6})");
    private static final Pattern AMP_BUNGEE_HEX      = Pattern.compile("(?i)&x(?:&[0-9a-f]){6}");
    private static final Pattern SEC_BUNGEE_HEX      = Pattern.compile("(?i)§x(?:§[0-9a-f]){6}");
    private static final Pattern MINI_HEX_TAG        = Pattern.compile("(?i)<#([0-9a-f]{6})>");
    private static final Pattern MINI_HEX_DOUBLE     = Pattern.compile("(?i)<##([0-9a-f]{6})>");
    private static final Pattern BARE_HEX            = Pattern.compile("(?i)(?<![0-9a-z#])#([0-9a-f]{6})(?![0-9a-z])");
    private static final Pattern AMP_CODES           = Pattern.compile("(?i)&([0-9a-fk-or])");
    private static final Pattern SEC_CODES           = Pattern.compile("(?i)§([0-9a-fk-or])");
    private static final Pattern ANY_MINI_TAG        = Pattern.compile("(?s)<[^>]+>");

    private static String normalizeToMiniMessage(String input, Set<Input> expects, Options opt) {
        ...
        if (expects.size() == 1 && expects.contains(Input.MINIMESSAGE) && !likelyHasLegacy(s)) {
            return s;
        }

        // 1) Optional § → & normalization
        final boolean mayContainSection = expects.contains(Input.LEGACY_SECTION)
                || expects.contains(Input.HEX_BUNGEE_SECTION)
                || expects.contains(Input.HEX_POUND);
        if (opt.normalizeSectionToAmpersand && mayContainSection && s.indexOf('§') >= 0) {
            s = s.replace('§', '&');
        }

        // 2) Hex variants → <#RRGGBB>
        if (opt.transformHex) {
            if (expects.contains(Input.HEX_POUND) && (indexOfAny(s, '&', '§', '#') >= 0)) {
                if (!opt.normalizeSectionToAmpersand) {
                    s = SECTION_POUND_HEX.matcher(s).replaceAll("<#$1>");
                }
                s = POUND_HEX.matcher(s).replaceAll("<#$1>");
            }
            if (expects.contains(Input.HEX_BUNGEE_AMP) && s.indexOf('&') >= 0) {
                s = replaceAmpersandBungeeHexToMini(s);
            }
            if (expects.contains(Input.HEX_BUNGEE_SECTION) && !opt.normalizeSectionToAmpersand && s.indexOf('§') >= 0) {
                s = replaceSectionBungeeHexToMini(s);
            }
            if (expects.contains(Input.HEX_MINI_DOUBLE) && MINI_HEX_DOUBLE.matcher(s).find()) {
                s = MINI_HEX_DOUBLE.matcher(s).replaceAll("<#$1>");
            }
            if (expects.contains(Input.HEX_PLAIN) && BARE_HEX.matcher(s).find()) {
                s = BARE_HEX.matcher(s).replaceAll("<#$1>");
            }
            // HEX_MINI already canon; no-op
        }

        // 3) Standard legacy codes → MiniMessage tags
        if (opt.transformStandardCodes) {
            if (expects.contains(Input.LEGACY_AMPERSAND) && s.indexOf('&') >= 0) {
                s = convertAmpCodesToMini(s);
            }
            if (expects.contains(Input.LEGACY_SECTION) && !opt.normalizeSectionToAmpersand && s.indexOf('§') >= 0) {
                s = convertSecCodesToMini(s);
            }
        }
    ...
    private static String replaceSectionBungeeHexToMini(String s) {
        Matcher m = SEC_BUNGEE_HEX.matcher(s);
        StringBuffer out = new StringBuffer(s.length());
        while (m.find()) {
            String seq = m.group(); // §x§F§F§F§F§F§F
            String hex = seq.replaceAll("(?i)§x|§", "");
    ...
    private static String convertAmpCodesToMini(String s) {
    ...
    /** Replace standard §-style legacy color/format codes with MiniMessage tags. */
    private static String convertSecCodesToMini(String s) {
        return s.replaceAll("(?i)§0", "<black>")
                .replaceAll("(?i)§1", "<dark_blue>")
                .replaceAll("(?i)§2", "<dark_green>")
                .replaceAll("(?i)§3", "<dark_aqua>")
    ...
    // Named colors/decorations → legacy code
    private static final Map<String, Character> TAG_TO_CODE;
    static {
        ...
        m.put("grey", '7'); // alias
        m.put("dark_gray", '8');
        m.put("dark_grey", '8'); // alias
        m.put("blue", '9');
        m.put("green", 'a');
        m.put("aqua", 'b');
        m.put("red", 'c');
        m.put("light_purple", 'd');
        m.put("purple", 'd'); // common alias
        m.put("yellow", 'e');
        m.put("white", 'f');
        // decorations
        m.put("bold", 'l');
        m.put("underlined", 'n');
        m.put("underline", 'n'); // alias
        m.put("strikethrough", 'm');
        m.put("italic", 'o');
        m.put("obfuscated", 'k');
        m.put("reset", 'r');
        TAG_TO_CODE = Map.copyOf(m);
    }
    ...
    private static String miniMessageToLegacy(String mm, char legacyChar, Options opt) {
        ...
        if (out.indexOf('<') >= 0 && MINI_HEX_DOUBLE.matcher(out).find()) {
            out = MINI_HEX_DOUBLE.matcher(out).replaceAll("<#$1>");
        }

        // 2) Hex <#RRGGBB>
        if (out.indexOf('#') >= 0 && MINI_HEX_TAG.matcher(out).find()) {
            Matcher hx = MINI_HEX_TAG.matcher(out);
            ...
            while (hx.find()) {
                ...
                if (opt.legacyOutputHexColors) {
                    ...
                    char code = nearestNamedLegacyCode(hex);
        ...
        for (var e : TAG_TO_CODE.entrySet()) {
            String tag = e.getKey();
        ...
        if (opt.legacyEmitResetOnClose && out.indexOf('<') >= 0 && out.contains("</")) {
    ...
    private static char nearestNamedLegacyCode(String hex) {
        ...
        for (var e : MC_COLORS.entrySet()) {
            ...
            if (d < best) { best = d; bestCode = e.getKey(); }
        }
        return bestCode;
    }

    private static double sq(int x) { return (double) x * x; }
    ...
    public static String stripLegacyCodes(String input) {
        if (input == null || input.isEmpty()) return input;
        String s = input;
        if (s.indexOf('&') >= 0) s = AMP_BUNGEE_HEX.matcher(s).replaceAll("");
        if (s.indexOf('§') >= 0) s = SEC_BUNGEE_HEX.matcher(s).replaceAll("");
        if (s.indexOf('#') >= 0) {
            s = POUND_HEX.matcher(s).replaceAll("");
            s = SECTION_POUND_HEX.matcher(s).replaceAll("");
        }
        if (s.indexOf('&') >= 0) s = AMP_CODES.matcher(s).replaceAll("");
        if (s.indexOf('§') >= 0) s = SEC_CODES.matcher(s).replaceAll("");
        return s;
    }
    ...
    public static String stripMiniMessageTags(String input) {
        ...
        if (s.indexOf('#') >= 0) s = MINI_HEX_TAG.matcher(s).replaceAll("");
        // Remove any <tag ...> or </tag>
        if (s.indexOf('<') >= 0) s = ANY_MINI_TAG.matcher(s).replaceAll("");
        return s;
    }
