package nl.hauntedmc.serverfeatures.features.capacity.internal.messaging;
...
/** Owns the logical Redis subscription for authoritative Capacity snapshots. */
public final class EventBusHandler {
    ...
    private final Capacity feature;
    ...
    public synchronized void subscribe(String channel) {
        if (closed.get()) {
            throw new IllegalStateException("Capacity event bus handler is closed");
        }
        if (subscription != null) {
            throw new IllegalStateException("Capacity event bus handler is already subscribed");
        }
        Subscription created = Objects.requireNonNull(
                redisBus.subscribe(
                        channel,
                        CapacitySnapshotMessage.TYPE,
        ...
        if (closed.get()) {
            created.unsubscribe();
            throw new IllegalStateException("Capacity event bus handler closed while subscribing");
        }
    ...
    public void disable() {
        ...
        try {
            ...
            if (shutdown == null) {
                feature.getLogger().warning("Capacity subscription returned no shutdown future.");
                return;
            }
                    ...
                    .whenComplete((ignored, throwable) -> {
                        if (throwable != null) {
                            feature.getLogger().warning(
                                    "Could not confirm Capacity subscription shutdown: "
            ...
            feature.getLogger().warning(
                    "Could not start Capacity subscription shutdown: " + rootMessage(exception)
            );
        }
    }

    private void handleIncoming(CapacitySnapshotMessage message) {
        if (closed.get()) {
            return;
        }
        long now = System.currentTimeMillis();
        CapacitySnapshotStore.ApplyResult result = store.apply(message, now);
        if (closed.get()) {
            store.clear();
            return;
        }
        if (result == CapacitySnapshotStore.ApplyResult.INVALID && claimInvalidWarning(now)) {
            feature.getLogger().warning(
                    "Ignored an invalid Capacity snapshot; repeated warnings are rate-limited."
            );
        }
    }
